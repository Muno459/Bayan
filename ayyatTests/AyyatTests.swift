...
struct VocabularyStoreTests {
    ...
    @Test("Word exposure increments count but never auto-promotes (active engagement only)")
    @MainActor
    func exposureTracking() {
        ...
        // Passive scrolling should NOT promote, even after 50 sightings —
        // promotion requires real engagement (tap / quiz / pronunciation).
        for _ in 0..<49 { store.recordExposure(for: word) }
        #expect(store.wordStates[10]?.exposureCount == 50)
        #expect(store.wordStates[10]?.masteryLevel == .unseen)

        // Active engagement (tap) moves the word forward.
        store.recordTap(for: word)
        // .unseen → tap does not auto-bump because recordTap only promotes
        // .introduced → .learning. So tapping an unseen word stays unseen.
        #expect(store.wordStates[10]?.masteryLevel == .unseen)
    }
    ...
    func promoteAndDemote() {
        // VocabularyStore persists to UserDefaults; previous test runs may
        // have left state for word id 20. Clear the slate so the assertions
        // are starting from .unseen.
        UserDefaults.standard.removeObject(forKey: "bayan_wordStates")
        let store = VocabularyStore()
...
struct UserStoreTests {
    ...
    func sessionLifecycle() async throws {
        ...
        // Sessions under 5s are dropped as noise. Wait long enough to clear
        // the threshold so the lifecycle test exercises the persisted path.
        try await Task.sleep(for: .seconds(5.2))

        store.endCurrentSession(lastVerseKey: "1:7")
        #expect(store.activeSession == nil)
        #expect(store.sessions.count == initialCount + 1)
        #expect(store.sessions.last?.endVerseKey == "1:7")
    }

    @Test("Sessions under 5 seconds are dropped as noise")
    @MainActor
    func shortSessionDropped() {
        let store = UserStore()
        let initialCount = store.sessions.count
        store.startSession(chapterId: 1, verseKey: "1:1")
        store.endCurrentSession()  // immediate end ≈ 0s
        #expect(store.sessions.count == initialCount)
    }
}
