...
final class TokenManager {
    ...
    private func requestToken() async throws -> String {
        ...
        // Client_credentials is now only used for endpoints that genuinely
        // need an app-level token — currently just Quran Reflect's
        // /quran-reflect/v1/posts/feed (community Lessons & Reflections),
        // which requires `post` scope. All actual Quran content reads go
        // through the public api.quran.com mirror with no token at all.
        request.httpBody = "grant_type=client_credentials&scope=post".data(using: .utf8)
        ...
        guard httpResponse.statusCode == 200 else {
            ...
            dlog("[ayyat] Token error \(httpResponse.statusCode): \(body)")
